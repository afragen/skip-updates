 ...
 */
class SU_List_Table extends \WP_List_Table {
	 ...
	 *
	 * @return array|void An associative array containing all the bulk actions: 'slugs'=>'Visible Titles'
	 **************************************************************************/
	public function get_bulk_actions() {
		$actions = [
			'delete' => esc_html__( 'Delete', 'skip-updates' ),
		];
			...
			foreach ( $slugs as $slug ) {
				foreach ( self::$options as $key => $option ) {
					if ( in_array( $slug, $option, true ) ) {
						...
						update_site_option( 'skip_updates', self::$options );
					}
				}
			}
		}
		if ( 'edit' === $this->current_action() ) {
			wp_die( esc_html__( 'Items would go to edit when we write that code.', 'skip-updates' ) );
		}
	}
	...
	private function check_nonce() {
		...
		if ( ! $nonce_exists ) {
			wp_die( esc_html__( 'A nonce was not properly set. Please report an issue.', 'skip-updates' ) );
		}
		if ( ! wp_verify_nonce( sanitize_key( wp_unslash( $_REQUEST[ $nonce ] ) ), $action ) ) {
			wp_die( esc_html__( 'Your nonce did not verify.', 'skip-updates' ) );
		}
	}
	...
	public function render_list_table() {
		...
		$this->prepare_items();
		echo '<div class="wrap">';
		echo '<h2>' . esc_html__( 'Skip Updates List Table', 'skip-updates' ) . '</h2>';
